...
# Function to remove/forget files properly
chezmoi_forget_or_destroy() {
  local filepath="$1"
  echo "🗑️ Removing file from tracking: $filepath"

  # First try forget (safer, keeps the file)
  if chezmoi forget "$filepath"; then
    return 0
  fi

  # If forget fails, try destroy (more aggressive, removes the file)
  echo "${YELLOW}Forget failed, trying destroy...${RESET}"
...
process_selected_files() {
  ...
  while IFS= read -r line; do
    ...
    M | A)
      ...
      ;;
    R | D | DA)
      # Handle removed, deleted, and renamed files
      chezmoi_forget_or_destroy "$filepath" || {
